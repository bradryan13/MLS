...
if ( !class_exists( 'WP_Widget_Ultimate_Posts' ) ) {
	...
	class WP_Widget_Ultimate_Posts extends WP_Widget {
		...
		function WP_Widget_Ultimate_Posts() {
			...
			$widget_ops = array( 'classname' => 'widget_ultimate_posts', 'description' => __( 'Displays list of posts with an array of options', 'usar' ) );
			$this->WP_Widget( 'sticky-posts', __( 'Ultimate Posts', 'usar' ), $widget_ops );
			...
			load_plugin_textdomain('usar', false, basename( dirname( __FILE__ ) ) . '/languages' );
...
?>"><?php the_category(', '); ?></span>
							...
							<?php if ( $instance['show_tags'] ) : ?>
								<p class="post-tags">
									<span class="post-tags-label"><?php _e('Tags', 'usar'); ?>:</span>
		...
		function form( $instance ) {
			...
			if ( $instance ) {
				...
				$thumb_crop = 1;
				$excerpt_length = 10;
				$excerpt_readmore = __('Read more &rarr;', 'usar');
				$order = 'DESC';
				$orderby = 'date';
				$morebutton_text = __('View More Posts', 'usar');
			...
			<p><label for="<?php echo $this->get_field_id( 'title_link' ); ?>"><?php _e( 'Title URL', 'usar' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Show published date', 'usar' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id( 'show_time' ); ?>"><?php _e( 'Show published time', 'usar' ); ?></label>
			</p>
			...
			<p>
				...
				<?php _e( 'Show read more link', 'usar' ); ?>
				</label>
			</p>
			...
			<?php if ( function_exists('the_post_thumbnail') && current_theme_supports( 'post-thumbnails' ) ) : ?>
				...
				<p>
					...
					<label for="<?php echo $this->get_field_id( 'show_thumbnail' ); ?>"><?php _e( 'Show thumbnail', 'usar' ); ?></label>
				</p>

				<p>
					<label><?php _e('Thumbnail size', 'usar'); ?>:</label>
					<br />
					<label for="<?php echo $this->get_field_id('thumb_w'); ?>">
						<?php _e('W', 'usar'); ?>: <input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id('thumb_w'); ?>" name="<?php echo $this->get_field_name('thumb_w'); ?>" value="<?php echo $thumb_w; ?>" />
					</label>
					<label for="<?php echo $this->get_field_id('thumb_h'); ?>">
						<?php _e('H', 'usar'); ?>: <input class="widefat" style="width:40%;" type="text" id="<?php echo $this->get_field_id('thumb_h'); ?>" name="<?php echo $this->get_field_name('thumb_h'); ?>" value="<?php echo $thumb_h; ?>" />
					</label>
				</p>

				<p>
					<label><?php _e('Crop mode', 'usar'); ?>:</label>
					<br />
					<select id="<?php echo $this->get_field_id('thumb_crop'); ?>" name="<?php echo $this->get_field_name('thumb_crop'); ?>">
						<option value="1"<?php if($thumb_crop == 1) echo ' selected'; ?>><?php _e('Scale &amp; Crop to Fit', 'usar'); ?></option>
						<option value="0"<?php if($thumb_crop == 0) echo ' selected'; ?>><?php _e('Stretch to Fit', 'usar'); ?></option>
						<option value="2"<?php if($thumb_crop == 2) echo ' selected'; ?>><?php _e('Proportional Scale', 'usar'); ?></option>
						<option value="3"<?php if($thumb_crop == 3) echo ' selected'; ?>><?php _e('Proportional Scale No Borders', 'usar'); ?></option>
					</select>
				</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_morebutton'); ?>"> <?php _e('Show more button', 'usar'); ?></label>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('morebutton_text'); ?>"><?php _e( 'More button text', 'usar' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('morebutton_url'); ?>"><?php _e( 'More button URL', 'usar' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('atcat'); ?>"> <?php _e('Show posts only from current category', 'usar');?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_cats'); ?>"> <?php _e('Show post categories', 'usar'); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_tags'); ?>"> <?php _e('Show post tags', 'usar'); ?></label>
			</p>

			<p>
			<label for="<?php echo $this->get_field_id('cats'); ?>"><?php _e( 'Select categories', 'usar' ); ?>:</label>
			...
			<p>
			<label for="<?php echo $this->get_field_id('types'); ?>"><?php _e( 'Select post types', 'usar' ); ?>:</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id('sticky'); ?>"><?php _e( 'Sticky posts', 'usar' ); ?>:</label>
				<select name="<?php echo $this->get_field_name('sticky'); ?>" id="<?php echo $this->get_field_id('sticky'); ?>" class="widefat">
					<option value="show"<?php if( $sticky === 'show') echo ' selected'; ?>><?php _e('Show All Posts', 'usar'); ?></option>
					<option value="hide"<?php if( $sticky == 'hide') echo ' selected'; ?>><?php _e('Hide Sticky Posts', 'usar'); ?></option>
					<option value="only"<?php if( $sticky == 'only') echo ' selected'; ?>><?php _e('Show Only Sticky Posts', 'usar'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e( 'Order by', 'usar' ); ?>:</label>
				<select name="<?php echo $this->get_field_name('orderby'); ?>" id="<?php echo $this->get_field_id('orderby'); ?>" class="widefat">
					<option value="date"<?php if( $orderby == 'date') echo ' selected'; ?>><?php _e('Published Date', 'usar'); ?></option>
					<option value="title"<?php if( $orderby == 'title') echo ' selected'; ?>><?php _e('Title', 'usar'); ?></option>
					<option value="comment_count"<?php if( $orderby == 'comment_count') echo ' selected'; ?>><?php _e('Comment Count', 'usar'); ?></option>
					<option value="rand"<?php if( $orderby == 'rand') echo ' selected'; ?>><?php _e('Random'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('order'); ?>"><?php _e( 'Order', 'usar' ); ?>:</label>
				<select name="<?php echo $this->get_field_name('order'); ?>" id="<?php echo $this->get_field_id('order'); ?>" class="widefat">
					<option value="DESC"<?php if( $order == 'DESC') echo ' selected'; ?>><?php _e('Descending', 'usar'); ?></option>
					<option value="ASC"<?php if( $order == 'ASC') echo ' selected'; ?>><?php _e('Ascending', 'usar'); ?></option>
				</select>
			</p>

			<p class="credits"><small><?php _e('Developed by', 'usar'); ?> <a href="http://pomelodesign.com">Pomelo Design</a></small></p>
